...
import { addColAfterCommand, addRowAfterCommand, createTable, strikethroughSchema } from '@milkdown/kit/preset/gfm';
import {
	addBlockTypeCommand,
	blockquoteSchema,
	bulletListSchema,
...
import { deleteColumn, deleteRow, deleteTable, isInTable } from '@milkdown/kit/prose/tables';
...
export const MarkdownNoteEditor = forwardRef<MarkdownNoteEditorHandle, MarkdownNoteEditorProps>(function MarkdownNoteEditor(
	...
	useImperativeHandle(
		...
		() => ({
			...
			insertMarkdown(markdown: string) {
				const nextMarkdown = insertMarkdownAtSelection(crepeRef.current, markdown, latestValueRef.current);
				latestValueRef.current = nextMarkdown;
				onChangeRef.current(nextMarkdown);
				return nextMarkdown;
			},
			replaceMarkdown(from: string, to: string) {
				const nextMarkdown = replaceMarkdownText(crepeRef.current, latestValueRef.current, from, to);
				latestValueRef.current = nextMarkdown;
				onChangeRef.current(nextMarkdown);
				return nextMarkdown;
			},
	...
	useEffect(() => {
		...
		const markToolbarEdit = (event: Event) => {
			if (event.target instanceof Element && event.target.closest('.milkdown-top-bar')) {
				hasUserEdited = true;
			}
		};
		...
		crepe.on((listener) => {
			listener.markdownUpdated((_, markdown) => {
				...
				if (isInitialMount || !hasUserEdited) {
		...
		root.addEventListener('beforeinput', markUserEdit, true);
		root.addEventListener('paste', markUserEdit, true);
		root.addEventListener('drop', markUserEdit, true);
		root.addEventListener('pointerdown', markToolbarEdit, true);
		root.addEventListener('click', markToolbarEdit, true);
		...
		return () => {
			disposed = true;
			root.removeEventListener('beforeinput', markUserEdit, true);
			root.removeEventListener('paste', markUserEdit, true);
			root.removeEventListener('drop', markUserEdit, true);
			root.removeEventListener('pointerdown', markToolbarEdit, true);
			root.removeEventListener('click', markToolbarEdit, true);
	...
	return (
		<div className="markdown-editor-shell" onKeyDownCapture={handleKeyDownCapture} onPasteCapture={handlePasteCapture}>
...
function replaceMarkdownText(crepe: Crepe | null, fallback: string, from: string, to: string) {
...
function isUnderlineHtmlNode(node: unknown) {
	return typeof node === 'object' && node !== null && (node as { type?: unknown }).type === 'html' && getUnderlineHtmlTag(node) !== null;
}
...
const topBarIcons = {
	...
	math: '<svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24"><path fill="currentColor" d="M7 19v-.808L13.096 12L7 5.808V5h10v1.25H9.102L14.727 12l-5.625 5.77H17V19z"/></svg>',
...
function ensureTopBarRows(topBar: Element) {
	...
	try {
		...
		[controls.slice(0, rowTwoStart), controls.slice(rowTwoStart, rowThreeStart), controls.slice(rowThreeStart)].forEach(
			(rowControls, index) => {
...
function isTopBarControlElement(element: Element): element is HTMLElement {
	return (
		element instanceof HTMLElement && (element.classList.contains('top-bar-heading-selector') || element.classList.contains('top-bar-item'))
	);
}

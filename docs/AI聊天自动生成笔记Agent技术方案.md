...
这个能力属于轻量 agent：AI 在聊天中识别“生成/整理/保存为笔记”的意图，并准备一个受控工具动作 `create_note`。但工具不会立刻写入笔记，而是先生成一条待确认 assistant 消息。消息正文先展示候选笔记 Markdown 内容，例如：
...
然后这条消息下方 `.ai-message-actions` 再显示确认问题：

```text
需要我帮你生成一篇「今日待做」的笔记吗？
```

只有用户点击“需要”后，后端才真正创建笔记；点击“不需要”则只标记本次建议已拒绝，不创建笔记。用户也可以不点击任何按钮继续聊天，pending 状态会保留在这条消息上。
...
- 该 assistant message 正文先渲染候选 Markdown；`.ai-message-actions` 显示“需要我帮你生成一篇「今日待做」的笔记吗？”以及“需要 / 不需要”。
...
- 生成候选标题和 Markdown。
- 先把候选 Markdown 作为 assistant 正文展示给用户，再询问是否需要生成该标题的笔记。
- 用户确认后再创建笔记。
...
当后端返回 `note_pending` 时，候选笔记的 `title` 和 `markdown` 已经写入该 assistant message 的 `metadata`。后续用户继续聊天不会重新生成或修改这条 pending message 的候选内容。

用户稍后点击“需要”时，前端会携带这条确认消息自己的 `messageId` 调用确认接口。后端只根据该 `messageId` 对应的 metadata 创建笔记。因此：

- 生成 pending 之前的聊天上下文会影响候选笔记内容。
- pending 生成之后的新聊天不会影响这条候选笔记内容。
- 如果后续聊天又触发了一次生成笔记意图，会产生另一条独立的 pending message。
...
如果识别到 `create_note`，返回候选笔记 Markdown 正文，并带响应头：

```http
X-Floating-Notes-Action: note_pending
X-Floating-Notes-Message-Id: <assistantMessageId>
X-Floating-Notes-Note-Title: <urlEncodedTitle>
```

前端用这个 message id 绑定确认按钮，用标题 header 渲染确认问题。
...
`ChatApp.tsx` 保持现有聊天发送入口。发送后先 yield 一条 running 状态的本地 loading 消息：普通聊天显示“正在思考...”，疑似生成笔记请求显示“正在整理待确认笔记...”。收到真实响应后，用响应正文替换 loading。

收到 `X-Floating-Notes-Action: note_pending` 后，把 `messageId`、`threadId` 和 `X-Floating-Notes-Note-Title` 写入 assistant-ui 的 `metadata.custom`。
...
- 前端先显示 running loading；pending assistant message 正文显示候选 Markdown，只替换 `.ai-message-actions` 为确认栏。
...
- 明确创建：
  ...
  - 应返回候选 Markdown 正文、`note_pending` header 和标题 header。
  - notes 表仍为空。
...
- 发送等待期间显示明显 loading 动画。
- pending assistant message 正文显示候选 Markdown，下方只显示确认问题和“需要 / 不需要”。
